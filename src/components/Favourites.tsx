...
const Favourites = () => {
  ...
  return (
    <section id="favorites" className="flex flex-col pl-5 pb-10 pt-2">
      ...
      <div className="flex justify-start flex-col pb-4">
        ...
        <ul className="flex w-full pb-5 flex-wrap">
          {seasonsList.map((season, index) => (
            <li key={`${season}${index}`} onClick={() => setSelectedLabel(`${season}`)} className={`flex items-center space-x-6 ${ index !== seasonsList.length - 1 ? 'pr-[89px]' : 'pr-[10px]'} cursor-pointer`}>
